...
**功能已可用并已上生产路径**：手机上通过 `http://<服务器IP>:17933` 经自家中转服务器控制本机 DSH，能对话、看会话、传文件、跑命令。移动端布局已做过三轮优化（最新一轮把适配面从「主流程」扩到**全组件**：通用原语、工具行、轨迹 tab、右栏全屏、手势），**回归断言全绿（含 WebKit 引擎的 iPhone 冒烟）**。

> ⚠️ **DSH 换版本了（0.1.7-alpha.2）**：跑的不再是 Electron 打包版，而是 npm 安装的 `@deepseek-ai/dsh`。端口、部分 CSS-module hash、工具行标签（`Pwsh` → `运行命令`）都变了。适配层已针对新版复查并修复（见 [5.4](#54-dsh-版本差异与待适配)），仍有一项待适配（工具行展开区）。
...
最近一次结果（DSH 0.1.7，不含 `--webkit`）：

```
共 54 项，失败 0 项，已知可接受 2 项
```
...
| 代理日志 | `Get-Content access.log -Tail 20` | 有 `upstream 可用/切换为 …`、`minted DSH session cookie`、`listening` |
| 上游端口对不对 | 日志里的 `upstream …` | 与 `$env:DSH_WEB_URL` 的端口一致（不一致时代理会自己发现并切换） |
| 公网三层 | 裸访问 `401` / 带 `?token=` `303` / 带 Cookie `200` | 见 2.2 |

> **重启 DSH 后按顺序查**：① 代理还在不在（19390）→ 不在就 `start-all.bat`；② 隧道还在不在（服务器 17933）→ 不在就重启隧道；③ 上游端口（日志里的 `upstream`）→ 代理会自动发现，正常会看到一行 `upstream 切换为 …`。
...
DSH 从 Electron 打包版切到 **npm 安装的 `0.1.7-alpha.2`** 后，实测到的差异（都已复查）：

| 差异 | 影响 | 状态 |
|---|---|---|
| **Web 端口变了**（19387 → 3080） | 代理写死的上游失效 → 远程 502 | ✅ 已修：代理自动发现（[D6](#七关键设计决策)） |
| **`data-rightbar-collapsed` 开关时一直存在** | 按属性判断右栏状态永远错 | ✅ 已修：改认「收起右侧边栏」按钮是否在视口内 |
| **右栏"关闭态"也把面板容器留在屏内**（内容 `translateX(420px)` 推出屏幕） | 按容器宽度判断会把"关着"当"开着" → 汉堡被误藏 | ✅ 已修：同上，认语义按钮 |
| **开关右栏只改内联 `style`** | MutationObserver（只监听 class）不触发 → 状态不刷新 | ✅ 已修：`attributeFilter` 加 `style` + 800ms 低频兜底巡检 + sync 幂等 |
| **会话行中间挂了 `_iconButton`** | 点行中心命中操作按钮 → 不触发"选中后自动收起" | ✅ 行为正确；审计改为点标题（`_title`） |
| **选会话时侧栏重渲染** | +120ms 时切换按钮可能已脱离文档 → 收起失败 | ✅ 已修：改成最多 4 次重试 |
| **工具行标签本地化**（`Pwsh` → `运行命令`） | 审计按 `Pwsh` 找不到行 | ✅ 已修：按中英文标签匹配 |
| **工具行展开区类名全变**（`_bodyWrap`/`_block`/`_copyButton`/`_inspectButton` 全为 0） | 那几条适配规则在新版成了空操作；展开区适配待做 | ⚠️ **待适配**：审计已降级为明确 WARN；新版类名线索 `O_Ebla_*`（工具卡）、`ztWv_q_*`（行）、复制钮 `_action[aria-label="复制"]`（已补 32px 点击目标） |
| CSS-module hash 全换（`ONzo8q` → `EvIC1a` 等） | — | ✅ 无影响：适配层只认 local 名与语义属性（[MOBILE.md 第四节](MOBILE.md)），契约断言全过 |

> 教训：**升级 DSH 后第一件事是跑审计**。这次 9 项差异里，契约断言（frame/sidebarCol/sessionRow/composerSeat…）全过，说明「只认 local 名 + 语义属性」的选择器策略是对的；出问题的都是**行为/状态判断**（端口、属性语义、动画残留）。
...
| DSH GUI 端口 | **动态**（实测重启后 `19387` → `3080`）。代理自动发现，配置里的 `upstream` 只是候选提示 |
| 代理监听 | `127.0.0.1:19390`（固定） |
...
**D6. 上游端口动态发现，而不是写死在配置里。**
DSH 的 Web 端口**每次重启都可能变**（实测 `19387` → `3080`），写死的话 DSH 每重启一次远程入口就 502 一次。代理启动时先试配置/`DSH_WEB_URL`，不通就扫本机 LISTEN 端口 + 常见端口，用 DSH 的特征响应（`401` + `dsh web authentication required`）认出来，并**并发探测**（串行要 6 秒以上）。
👉 代价：多了一次本机端口探测（约 0.8 秒，只在启动或连接失败时发生）。端口变化时会自动重签 cookie（cookie 名与内容都绑定 authority）。

**D7. 代理与隧道必须是独立进程，不能挂在 DSH 下面。**
从 DSH 的终端里启动它们，它们就是 DSH 的子进程 —— **DSH 一重启就被连带杀掉**（实测踩过：重启 DSH 后 17933 的隧道消失、只剩 bot 项目的 17932 隧道在跑）。所以用独立启动器：Windows 的登录自启 VBS、macOS 的 launchd、或 `start-all.bat`（都是 `start` / `Run` 方式脱离父进程）。
