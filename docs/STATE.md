...
从 DSH 的终端里启动它们，它们就是 DSH 的子进程 —— **DSH 一重启就被连带杀掉**（实测踩过：重启 DSH 后 17933 的隧道消失，只剩另一个服务的隧道在跑）。所以用独立启动器：Windows 的登录自启 VBS、macOS 的 launchd、或 `start-all.bat`（都是 `start` / `Run` 方式脱离父进程）。
