...
它们是 DSH 的子进程，DSH 一重启整棵进程树就没了。实测踩过：重启 DSH 后 17933 的隧道消失，服务器上 `ss -ltn` 查不到监听（公网表现为**连接被拒绝**，区别于超时=安全组/VPN），而机器上还活着的那个 ssh 是**另一个服务的**隧道（转发的是别的端口）。
...
开源前第一轮只扫了服务器 IP / 邮箱 / 域名 / 个人路径 → 全 clean，看着可以发了。第二轮把口径放宽（所有非 loopback IPv4、云厂商与面板工具字样、另一个私有项目的名字与端口、安全组端口清单）→ 又抓出一批：**另一项目的引用与端口指纹、你服务器的安全组放行清单、个人时间线细节**。
→ 顺序应是：先列「所有能定位到我的东西」，再逐个决定泛化还是删除；并且把检查**固化成脚本 + pre-commit 钩子**，别靠记性（[README §十一](../README.md)）。
→ 通用模式认不出「只属于你的词」（项目名、端口、厂商标识）—— 要显式写进 `remote.config.json` 的 `scrubWords`。**本轮就栽在这**：清理过的三个串在我后来写文档时又被带回来了，而闸门当时没有这些词，直到手工复扫才发现。
